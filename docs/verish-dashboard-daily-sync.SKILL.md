...
- 시트에는 탭이 3개 있고, `read_file_content`는 이 세 표를 이 순서로 이어붙인 마크다운 텍스트 하나로 반환한다:
  1. `시트1` (SKU 단위) — A주 시작일 B주 종료일 C연령대 D순위 E**상품명(SKU 정제)** F판매수량 G주문건수
  2. `상품명TOP10` (판매 상품명 단위) — 컬럼은 위와 동일하지만 E열 헤더가 그냥 **상품명** (괄호 없음) — 이 헤더 문구 차이로 두 표를 구분한다
  3. `일별매출` — A날짜 B주문건수 C합계매출 D객단가
  각 표 헤더 1행 + 데이터. `regenerate_dashboard.mjs`가 각 표의 헤더 행 문구를 보고 어느 표인지 자동 판별해서 나눠 담으므로, 이 작업은 세 표를 빠짐없이 그대로 옮겨 적기만 하면 된다 (표 순서가 바뀌어도 헤더 문구로 판별하니 문제없음).
- `index.html` 안의 `var RAW_SKU = { ... };` / `var RAW_PRODUCT = { ... };` 블록은 각각 `// RAW_SKU_DATA_START/END`, `// RAW_PRODUCT_DATA_START/END` 주석 사이에 있고, `regenerate_dashboard.mjs`가 자동으로 다시 만든다 (직접 손으로 편집하지 말 것). 대시보드 화면에는 "집계 기준" 토글(SKU/상품명)이 있어 사용자가 둘 중 하나를 골라 볼 수 있다.

## 1. 시트 내용 가져오기
Google Drive MCP의 `read_file_content` 도구를 fileId `1r_uz6bSUNHbyF49lsUqhwGlEb17hbKWHSUzn6K1ocVU` 로 호출한다. 반환된 마크다운 표 텍스트 전체(세 표 모두)를 Write 도구로 다음 경로에 그대로 저장한다 (절대경로로 바로 지정, cd 불필요):
...
```
- 정상이면 `updated index.html: SKU N week(s)/M row(s), 상품명 N week(s)/M row(s), ... revenue days: R` 라고 출력된다.
...
- **SKU 쪽은 row 수가 정상인데 상품명 쪽이 `0 week(s)/0 row(s)`로 나오면, `상품명TOP10` 표가 `sheet_export.txt`에서 누락된 것 — 1단계에서 세 표 중 두 번째 표를 빠뜨렸을 가능성이 높다.** 이 경우 `grep -c "상품명" sheet_export.txt`(헤더 문구 존재 확인 — "상품명(SKU 정제)"와 "상품명" 둘 다 이 문자열을 포함하므로 2 이상이어야 정상)로 확인 후, 없으면 1단계로 돌아가 `read_file_content` 결과 전체(세 표 모두)를 빠짐없이 다시 저장하고 2단계를 재실행한다.
- **`revenue days: 0`이 나오면 매출 표(일별매출: 날짜/주문건수/합계매출/객단가)가 `sheet_export.txt`에서 누락된 것 — 1단계에서 시트 텍스트를 옮겨 적을 때 세 번째 표를 빠뜨렸을 가능성이 높다 (과거 실제로 이 실수가 있었음).** 이 경우:
  1. `sheet_export.txt`에 `합계매출` 문자열이 있는지 확인한다 (예: `grep -c "합계매출" sheet_export.txt`). 없으면 1단계로 돌아가 `read_file_content` 결과 전체(세 표 모두)를 빠짐없이 다시 저장하고 2단계를 재실행한다.
...
## 4. 완료 보고
사용자에게 다음을 간단히 요약해서 보고한다: 몇 주차 데이터가 반영됐는지, SKU/상품명 각각 총 행 수, **매출 차트 데이터 일수(revenue days)**, push 여부(또는 변경 없음), 스킵된 행이 있었다면 그 내용, 실패했다면 어디서 왜 실패했는지. 배포 URL(https://dhlee-blip.github.io/verish-age-top10-dashboard/)도 함께 알려준다.
